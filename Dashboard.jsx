...
const PD_COL = {
  date: 1, rmName: 29, sortingKg: 4, ipBatch: 32, partyBatch: 33, opBatch: 34,
  boxes: 36, machine: 30, fgCode: 41, output: 38, kgWorkerStd: 46,
  ladies: {
...
function EfficiencyPanel({ prodData, payRates, productMasterByCode }) {
  ...
  const regularStats = useMemo(() => {
    ...
    for (const row of filtered) {
      ...
      if (!groups[key]) groups[key] = { key, weightedManpower: 0, qty: 0, exactStd: 0 };
      const w = getWeights(payRates, row.month);
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
        ...
        <FilterBar month={month} setMonth={setMonth} months={months} machine={machine} setMachine={setMachine} machines={machines} category={category} setCategory={setCategory} categories={categories} product={product} setProduct={setProduct} products={products} groupBy={groupBy} setGroupBy={setGroupBy} />
      </div>
      ...
      <Section title="Regular Production — Month-wise Trend" icon={TrendingUp}>
...
function FilterBar({ month, setMonth, months, machine, setMachine, machines, category, setCategory, categories, product, setProduct, products, groupBy, setGroupBy }) {
  return (
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
      <Dropdown label="Month" value={month} onChange={setMonth} options={[['all', 'All months'], ...months.map(m => [m, monthLabel(m)])]} />
